...
var (
	postDBTypes = map[string]string{`ID`: `integer`, `AuthorID`: `integer`, `Content`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
